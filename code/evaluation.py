...
# Some of the models have capitalization and some do not, which was resolved with this function:
...
# The floret model doesn't have index_to_key. As such, it was loaded separately
def load_model(p):
...
# K-Means is sensitive to random itilitalization, so it was run 10 times
...
def relatedness_eval(embeddings, return_raw=False):
    ...
    for w1, w2 in zip(df_cat['word1'], df_cat['word2']):
        ...
        if r1 is not None and r2 is not None:
            ...
            # If a word is not in the embeddings, we add a NaN value (this is only 
            # relevant for the .vec file, since .bin models can fall back on subword vectors)
...
def steiger_z_dependent_corr(r_xy, r_xz, r_yz, n):
    ...
    """
    # rm is just the average of the two correlations, used below as an estimate of their typical size
    rm = (r_xy + r_xz) / 2

    # f and h are correction terms from Steiger's formula that account for the fact that
    # r_xy and r_xz aren't independent, since they're both correlated with the same human scores
    f = (1 - r_yz) / (2 * (1 - rm**2))
    h = (1 - f * rm**2) / (1 - rm**2)

    # z is the test statistic — basically the difference between the two correlations,
    # scaled by how much variation we'd expect if there were really no difference between them
...
def compare_purity(purity_runs_a, purity_runs_b, label_a, label_b):
    ...
    """
    # ttest_rel expects matched pairs of observations — here, the same seed used for both models
...
def compare_relatedness(df_a, df_b, label_a, label_b):
    ...
    """
    # We only compare the word pairs both models could actually score, since OOV coverage can differ
    merged = df_a.merge(df_b, on=['word1', 'word2'], suffixes=('_a', '_b'))

    # Recompute each model's correlation with the human scores on this shared subset,
    # so we're comparing them on exactly the same set of word pairs
    ...
    # Steiger's test also needs to know how correlated the two models are with each other,
    # since that tells us how dependent the two correlations we're comparing really are
...
def compare_analogy(df_a, df_b, label_a, label_b):
    ...
    """
    # Match each question by its index, so we're comparing both models on the same analogy items
    merged = df_a.merge(df_b, on='index', suffixes=('_a', '_b'))

    # We can only include questions both models actually attempted —
    # if one model had an OOV word for that question, it can't be used in the comparison
    merged = merged[(merged['valid_a']) & (merged['valid_b'])]

    correct_a = merged['correct_a'].to_numpy()
    correct_b = merged['correct_b'].to_numpy()

    # McNemar's test only looks at the questions where the models disagreed —
    # cases where both got it right or both got it wrong don't tell us which model is better
