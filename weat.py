...
def print_neighbours(embeddings, display_name, n=N_NEIGHBOURS):
    """Print LGBT+ and neutral nearest neighbours side by side."""
...
if __name__ == "__main__":
    ...
    for stem, display_name in all_models.items():
        ...
        try:
            ...
            if stem in CAVE_MODELS:
                ...
                other_results[display_name] = entry

            print(f"  WEAT d={d:.4f}, p={pval:.4f}")

            # Nearest-neighbour inspection is only printed for CaVe models
            if stem in CAVE_MODELS:
    ...
    other_order = list(OTHER_MODELS.values())
    full_order  = cave_order + other_order

    all_results = {**cave_results, **other_results}

    plot_weat_summary(
        all_results, full_order,
